...
const OFFICIAL_SCHEDULE = [
    ...
    { id: "m16", t1: "إران", t2: "نيوزيلندا", grp: "1", stg: "group", dt: "2026-06-16T01:00:00Z", res: { s1: 2, s2: 2, penW: null, ps1: null, ps2: null } },
...
// الحصانة البرمجية القاطعة: تفحص وتفك أي نص متجمد قادم من قاعدة البيانات فوراً لضمان قراءة الـ uid الصحيح لجميع المشتركين
...
module.exports = async (req, res) => {
    ...
    try {
        if (route === 'auth') {
            ...
            if (action === 'login') {
                const cleanUser = username.trim().toLowerCase(); const userObj = await safeGet(`user:${cleanUser}`);
        ...
        if (route === 'predictions') {
            ...
            if (req.method === 'GET') {
                const userPreds = await safeGet(`preds:${userSession.uid}`);
